...
def updateHand(hand, word):
    ...
    """
    # TO DO ... <-- Remove this comment when you code this function
    # import copy            h=copy.deepcopy(hand)
    # h=dict(hand)
    h=hand.copy()
    for w in word:
        h[w]=h.get(w,0)-1
...
def isValidWord(word, hand, wordList):
    ...
    h=hand.copy()
    c=False
    if word in wordList:c=True
    a=getFrequencyDict(word)
    for i in a.keys():
        if not (i in h.keys() and h[i]>=a[i]):
            return False
    return c
#
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        if inp=='.':
            ...
            sc=getWordScore(inp, n)
            score+=sc
            print("\""+inp + "\" earned " + str(sc) + " points. Total score: " + str(score))
            print()
